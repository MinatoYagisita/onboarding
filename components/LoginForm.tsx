...
export function LoginForm({ orgSlug }: Props) {
  ...
  async function handleCodeSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/auth/passcode/verify", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email, organizationSlug: orgSlug, passcode: code, adminLogin: false }),
      });
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
  ...
  const inputClass =
    "rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 placeholder:text-gray-400 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/15";
  const btnClass =
    "mt-2 rounded-md bg-brand-600 px-4 py-2.5 text-sm font-medium text-white transition-colors hover:bg-brand-700 focus:outline-none focus:ring-2 focus:ring-brand-500 disabled:cursor-not-allowed disabled:bg-gray-300";
  ...
  return (
    <form onSubmit={handleCodeSubmit} className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-1">
        <label htmlFor="login-code" className="text-xs font-medium text-gray-700">
          パスコード（6桁）
        </label>
        <input
          id="login-code"
